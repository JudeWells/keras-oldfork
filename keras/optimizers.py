...
class DRPROP(Optimizer):
    ...
    def __init__(self, lr=0.001, decay=0., eta_plus = 1.2, eta_minus = 0.1,
                 eta_min=1e-2, eta_max=1e2, **kwargs):
        super(DRPROP, self).__init__(**kwargs)
        self.__dict__.update(locals())
        self.iterations = K.variable(0)
        self.lr = K.variable(lr)
        self.decay = K.variable(decay)
    ...
    def get_updates(self, params, constraints, loss):
        ...
        for p, g, a, pg, pp in zip(params, grads, alphas, prev_grads, prev_param):
            ...
            a_t = a * K.switch(change_above_zero, self.eta_plus,
                    K.switch(change_below_zero, self.eta_minus, 1.))
            a_clipped = K.clip(a_t, self.eta_min, self.eta_max)
            g = K.switch(change_above_zero, g,
                    K.switch(change_below_zero, 0., g))
            p_t = p - lr * K.sign(g) * a_clipped
            new_p = p_t #K.switch(K.equal(pg, 0.), pp, p_t)
            # apply constraints
            if p in constraints:
    ...
    def get_config(self):
        config = {'lr': float(K.get_value(self.lr)),
                  ...
                  'eta_min': float(self.eta_min),
                  'eta_max': float(self.eta_max)}
        base_config = super(DRPROP, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
